...
public class DefaultCallback implements Callback {
    ...
    private final Set<Integer> printedTaskIds = Collections.newSetFromMap(new ConcurrentHashMap<>());
    ...
    public synchronized void v2_playbook_on_task_start(Task task, boolean isConditional) {
        if (printedTaskIds.add(System.identityHashCode(task))) {
            System.out.println();
    ...
    public synchronized void v2_playbook_on_handler_stats(String handlerName) {
        if (printedTaskIds.add(System.identityHashCode(handlerName))) {
            System.out.println();
