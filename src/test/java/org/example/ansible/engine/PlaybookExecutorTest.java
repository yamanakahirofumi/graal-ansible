...
class PlaybookExecutorTest {
    ...
    void testHostFailureSkipsSubsequentTasks() {
        ...
        taskExecutor.registerModule("fail_on_host1", (args, become, context) -> {
        ...
        taskExecutor.registerModule("fail_if_var_true", (args, become, context) -> {
            if ("true".equals(args.get("should_fail"))) {
                return TaskResult.failure("Intentional failure");
            }
            return TaskResult.success(false, Map.of());
        });
        taskExecutor.registerModule("debug", (args, become, context) -> TaskResult.success(false, Map.of()));
