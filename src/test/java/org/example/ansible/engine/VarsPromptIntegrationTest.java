...
class VarsPromptIntegrationTest {
    ...
    @Test
    void testVarsPromptOverridesPlayVars() {
        String inventoryIni = "host1";
        Inventory inventory = new IniInventoryParser().parse(new ByteArrayInputStream(inventoryIni.getBytes(StandardCharsets.UTF_8)));
        ...
        // Mock PromptProvider returns something else
        playbookExecutor.setPromptProvider(promptDef -> "from_prompt_overriding_vars");

        // Level 13 (vars_prompt) should override Level 12 (play vars)
        Map<String, List<TaskResult>> results = playbookExecutor.execute(playbook, inventory);
        assertEquals("from_prompt_overriding_vars", results.get("host1").get(0).data().get("msg"));
    }

    @Test
    void testVarsFilesOverridesVarsPrompt() throws java.io.IOException {
        java.nio.file.Path tempDir = java.nio.file.Files.createTempDirectory("ansible_test");
        java.nio.file.Path varsFile = tempDir.resolve("external_vars.yml");
        java.nio.file.Files.writeString(varsFile, "my_var: from_vars_files");
        ...
        try {
            ...
            // Level 14 (vars_files) should override Level 13 (vars_prompt)
            Map<String, List<TaskResult>> results = playbookExecutor.execute(playbook, inventory, Map.of(), tempDir);
            assertEquals("from_vars_files", results.get("host1").get(0).data().get("msg"));
        } finally {
            java.nio.file.Files.deleteIfExists(varsFile);
