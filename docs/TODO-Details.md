...
### 5.1 [ ] SSH 踏み台サーバー (Bastion / Jump Host) 経由接続のサポート
- **概要**: ターゲットホストがプライベートネットワークにあり、踏み台サーバー（Bastion）経由でしか接続できない環境のサポート。Apache MINA SSHD を用いた Java ネイティブな多段 SSH トンネリング（ローカルポートフォワード等）の実装設計。
- **検討内容**:
    - `ansible_ssh_common_args` または `ansible_ssh_extra_args` 内の `ProxyCommand` / `ProxyJump` 構文のパース。または独自の `ansible_bastion_host` 等の変数のサポート。
    - Apache MINA SSHD で Bastion へのセッションを確立し、`session.startLocalPortForwarding` を用いて、ローカルの動的ポートからターゲットホストのポート 22 へのトンネルを掘る仕組みの実装詳細。
    - トンネルライフサイクルの管理（ターゲットへの接続開始時にトンネルを作成し、クローズ時にトンネルセッションとターゲットセッションの双方をクローズするリソースリーク防止策）。
    - 認証情報（公開鍵認証、パスワード認証）を踏み台とターゲットホストで個別に管理・適用する仕組み（複数の踏み台サーバー対応の要否と、認証情報解決の優先順位ルールの検討など、一貫性を重視して人間が主導して設計・決定する必要がある）。
