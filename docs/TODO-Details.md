...
### 3.21 [✓] 動的インベントリ - スクリプト方式 (Script mode) の実装
- **完了日**: 2026-07-27
- **概要**: 外部スクリプトを実行して JSON 形式のインベントリを取得する機能を実装。
- **解決策**: `ScriptInventoryProvider` を実装し、`_meta.hostvars` やグループ階層の解析をサポート。

### 3.22 [✓] インベントリ・ディレクトリのサポート
- **完了日**: 2026-07-27
- **概要**: インベントリソースとしてディレクトリを指定し、中のファイルを再帰的に読み込む機能を実装。
- **解決策**: `InventoryManager` を拡張し、再帰的探索、アルファベット順のソート、および標準的な除外ルールを適用。
