...
### 3.21 [✓] 動的インベントリ (Script mode) の統合
- **完了日**: 2026-07-26
- **概要**: 外部スクリプト（Python等）を実行し、その JSON 出力をインベントリとして取り込む機能を実装。
- **解決策**:
    - `ScriptInventoryProvider` を実装し、`--list` 引数によるスクリプト実行と Jackson による JSON 解析を統合。
    - `_meta.hostvars` によるホスト変数の効率的な取得をサポート。

### 3.22 [✓] インベントリディレクトリの再帰的サポート
- **完了日**: 2026-07-28
- **概要**: インベントリソースとしてディレクトリが指定された場合、配下のファイルを再帰的に探索して読み込む機能を実装。
- **解決策**:
    - `InventoryManager.loadInventory` においてディレクトリの再帰的走査ロジックを実装。
    - Ansible 互換のファイルフィルタリング（隠しファイルやバックアップファイルの除外）を統合。
