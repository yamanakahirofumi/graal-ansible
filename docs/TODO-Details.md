...
### 3.30 [✓] シリアル実行 (serial) のサポート
- **完了日**: 2026-06-27
- **概要**: `linear` 戦略において、一度に実行するホストの数を制限する（バッチ実行）機能の実装。
- **解決策**:
    - `Play` レコードに `serial` フィールドを追加し、`YamlParser` での解析をサポート。
    - `VariableManager` にバッチコンテキスト管理機能を導入し、`ansible_play_batch` マジック変数を動的に更新するように修正。
    - `LinearStrategy` において、`serial` の設定（整数、パーセンテージ、リスト）に基づいてホストを分割し、バッチごとにタスク実行とハンドラーフラッシュを行うロジックを実装。
