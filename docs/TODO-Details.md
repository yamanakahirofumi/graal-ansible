...
### 3.28 [✓] シリアル実行 (serial) のサポート
- **完了日**: 2026-07-29
- **概要**: `linear` 戦略において、一度に実行するホストの数を制限する（バッチ実行）機能の実装。
- **解決策**:
    - `Play` レコードへの `serial` フィールドの追加と解析。
    - `LinearStrategy` におけるバッチごとの実行ループ制御を実装。整数、パーセンテージ、リスト形式の指定をサポート。
