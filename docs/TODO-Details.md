...
### 3.28 [✓] 非同期タスク (async, poll) の実装
- **完了日**: 2026-06-20
- **概要**: タスクをバックグラウンドで実行し、完了を待機または後で確認する仕組みを実装。
- **解決策**:
    - `AsyncJobManager` を導入し、`~/.ansible_async/` でのジョブ状態の永続化を実装。
    - `TaskExecutor` において、`async` および `poll` パラメータに基づくバックグラウンド実行とポーリング制御を実装。
    - `async_status` モジュールによる状態確認をサポート。
...
### 6.3 [ ] 再帰的ハッシュマージ (hash_behaviour=merge) の実装
- **概要**: 辞書型の変数が重複した場合に、単なる置換ではなく再帰的にマージする機能の導入。
- **検討内容**:
    - `VariableManager` におけるマージロジックの実装。
    - 環境変数 `ANSIBLE_HASH_BEHAVIOUR` による動的な切り替え。

### 6.4 [ ] Python ベースのコールバックのサポート
