...
### 3.5 [✓] 実際の Ansible コレクションを使ったテストの実施
- **完了日**: 2026-07-26
...
### 3.6 [✓] Action Plugin の互換性向上 (Python-first)
- **完了日**: 2026-03-25
...
### 3.8 [✓] --extra-vars における JSON/YAML および @file 構文のサポート
- **完了日**: 2026-07-27
...
### 3.11 [✓] 独自 Jinja2 フィルターの拡充 (Ansible 互換)
- **完了日**: 2026-05-11
...
### 3.13 [✓] SnakeYAML 2.x による Playbook 解析の実装
- **完了日**: 2026-03-04
...
### 3.17 [✓] タスク制御機能（when, register, loop, handlers, block, retry, check_mode 等）の実装
- **完了日**: 2026-03-05
...
### 3.18 [✓] タスク制御キーワードの拡充 (environment)
- **完了日**: 2026-03-05
...
### 3.19 [✓] Ansible 互換性の維持レベル
- **決定事項**: **Ansible 13** で動くコレクションが動作することを目標とする。

### 3.20 [✓] 変数の優先順位（22段階）の完全な実装とテスト
- **完了日**: 2026-07-27
...
### 3.24 [✓] エラー時の即時停止 (any_errors_fatal) の実装
- **完了日**: 2026-06-05
...
### 3.26 [✓] インベントリ・プロバイダーの統合 (InventoryManager)
- **完了日**: 2026-06-12
...
### 3.27 [✓] ストラテジ・プラグイン（Strategy Plugins）の実装
- **完了日**: 2026-07-28
...
### 3.28 [✓] 再帰的ハッシュマージ (hash_behaviour=merge) の実装
- **完了日**: 2026-07-28
...
### 6.1 [ ] 失敗許容率 (max_fail_percentage) の実装
- **概要**: 特定のプレイまたはタスクにおいて、失敗したホストの割合が閾値を超えた場合に実行を停止する機能。
- **検討内容**:
    - `serial` バッチ実行時および `any_errors_fatal` 設定時との相互作用の定義。
    - 判定ロジック（計算式： `(失敗数 / ターゲット数) * 100 > max_fail_percentage`）のエンジンへの組み込み。

### 6.2 [ ] Python ベースのコールバックのサポート
