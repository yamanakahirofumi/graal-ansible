...
### 3.21 [✓] 動的インベントリ (スクリプト方式) の実装
- **完了日**: 2026-07-26
- **概要**: 外部スクリプトの実行と JSON 出力の解析による動的インベントリのサポート。
- **解決策**:
    - `ScriptInventoryProvider` を実装し、`--list` 引数によるスクリプト実行と Jackson による JSON 解析を統合。
    - `InventoryManager` による静的ファイルとスクリプトの透過的なマージを実現。
...
- **検討内容**:
    - 動的インベントリ（プラグイン方式）の統合実装。
    - ディレクトリ構造からのインベントリ再帰読み込みのサポート。
