...
### 3.21 [✓] 動的インベントリの統合 (Script mode)
- **完了日**: 2026-05-15
- **概要**: 外部スクリプト（JSON出力）による動的インベントリのサポート。
- **解決策**:
    - `InventoryProvider` インターフェースを導入し、`ScriptInventoryProvider` を実装。
    - `ProcessBuilder` によるスクリプト実行（`--list`）と Jackson による JSON 解析を統合。

### 3.22 [✓] インベントリ・ディレクトリのサポート
- **完了日**: 2026-05-16
- **概要**: 複数のインベントリソースを含むディレクトリの再帰的読み込み。
- **解決策**:
    - `InventoryManager` においてディレクトリの再帰的探索ロジックを実装。
    - アルファベット順の処理、隠しファイル・一時ファイルの除外ルールを適用。
