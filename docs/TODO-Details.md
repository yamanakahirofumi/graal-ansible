...
### 1.2 [ ] 実行戦略 (Strategy) の高度な制御
- **概要**: `linear` および `free` 戦略における並列度や実行順序の柔軟な制御。
- **検討内容**:
    - **forks の外部設定**: 現在固定値（5）となっている並列数を、CLI 引数 (`--forks`)、環境変数、または Play 内の設定から動的に変更可能にする設計。
    - **シリアル実行 (serial)**: `linear` 戦略において、一度に実行するホストの数を制限するバッチ実行のサポート。
    - **スロットリング (throttle)**: タスクやブロック単位での最大並列実行数の制限機能。
    - **ストラテジ固有のコールバック**: `free` 戦略等において、ホストごとに進捗が異なる場合の出力形式のさらなる最適化。

### 1.3 [ ] 変数マージ挙動 (hash_behaviour) の拡張
- **概要**: 辞書（Map）型変数の階層的なマージ機能のサポート。
- **検討内容**:
    - **hash_behaviour=merge**: 現在の「上位レベルによる完全上書き」に加え、`ANSIBLE_HASH_BEHAVIOUR=merge` 設定時に Map 変数を再帰的にマージするロジックの設計。
    - **パフォーマンスへの影響**: 22段階の優先順位解決において、再帰マージを導入した際の計算コストとキャッシュ戦略。
