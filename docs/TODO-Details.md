...
- **完了日**: 2026-05-31
- **概要**: 外部スクリプト（Python等）を実行して JSON 形式のインベントリを取得する仕組み。
- **解決策**:
    - `ScriptInventoryProvider` を実装し、`--list` 引数によるスクリプト実行と JSON 解析をサポート。
    - `InventoryManager` による統合管理を実現。

### 3.22 [✓] インベントリ・ディレクトリのサポート
- **完了日**: 2026-05-31
- **概要**: インベントリソースとしてのディレクトリ指定と、配下ファイルの再帰的・アルファベット順の読み込み。
- **解決策**:
    - `InventoryManager` においてディレクトリの再帰的探索とファイルフィルタリング（隠しファイル、バックアップファイルの除外）を実装。
    - ファイル名によるソート処理を導入し、読み込み順序を Ansible 互換に調整。
