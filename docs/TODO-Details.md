...
### 3.21 [✓] 動的インベントリ（スクリプト）の統合
- **完了日**: 2026-07-27
- **概要**: 外部スクリプトを用いた動的インベントリ取得のサポート。
- **解決策**:
    - `InventoryProvider` インターフェースと `InventoryManager` による抽象化レイヤーの実装。
    - `ScriptInventoryProvider` による外部 JSON インベントリ（`--list`）のパースとマージを実装済み。
