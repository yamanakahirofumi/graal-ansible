...
### [ ] 主要モジュールの実行ブリッジの実装
- **概要**: 実際の本家 Ansible モジュールを GraalPy 経由で呼び出すための効率的なブリッジ。
- **検討内容**:
    - `command`, `shell`, `file`, `copy`, `template` 等の主要モジュールの実行検証。
    - 入出力 JSON の Java Record へのマッピング最適化。
...
- **解決策**: [Native Image 最適化](implementation/Native-Image-Optimization.md) にて、エージェントによる自動生成と手動設定の併用方針を策定済み。
