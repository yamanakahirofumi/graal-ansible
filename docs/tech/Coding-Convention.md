...
### 1.3 名前付け規則
- クラス名は `UpperCamelCase`（例：`PlaybookParser`）を使用します。
- メソッド名、変数名は `lowerCamelCase`（例：`runTask`）を使用します。
...
本プロジェクトの Python コード（主に Java とのブリッジ部分）における記述基準を定義します。

### 4.1 型情報の付与 (Type Hints)
- すべての関数、メソッドの引数および戻り値には、PEP 484 に基づく型ヒント（Type Hints）を付与してください。
- `typing` モジュールを活用し、複雑なデータ構造（`List`, `Dict`, `Optional` 等）を明示してください。
- これにより、IDE での補完を有効にし、mypy 等の静的解析ツールによる検証を可能にします。

### 4.2 命名規則
- 関数名、変数名は `snake_case` を使用します。
- クラス名は `UpperCamelCase` を使用します。
- 定数は `UPPER_SNAKE_CASE` を使用します。

### 4.3 ドキュメンテーション
- 複雑なロジックを持つ関数には、Google スタイルまたは NumPy スタイルの docstring を記述してください。
