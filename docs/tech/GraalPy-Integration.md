...
| オプション | 設定値 | 目的 |
| :--- | :--- | :--- |
| `python.IsolateNativeModules` | `true` (Linux) | ネイティブモジュールの分離。Linux環境では安定のため `true` を使用（詳細は [5. 実行環境コンテキストの固定](../tech/Test-Expansion-Strategy.md#25-実行環境コンテキストの固定-execution-context-consistency) を参照）。 |
