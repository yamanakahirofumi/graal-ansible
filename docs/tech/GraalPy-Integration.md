...
| オプション | 設定値 | 目的 |
| :--- | :--- | :--- |
| `python.IsolateNativeModules` | `false` | ネイティブモジュールの分離を無効化し、特定のライブラリ（`ansible-core` 内）での再帰的インポートやシンボル解決の不整合を回避します。 |
| `python.PosixModuleBackend` | `java` | POSIX モジュールのバックエンドを Java 実装に切り替え、ファイル操作やシステムコールの安定性を向上させます。 |
