...
コールバックプラグインは、Playbook 実行中の各ライフサイクルイベント（Playの開始、タスクの完了、失敗等）をフックし、標準出力へのログ表示や外部システムへの通知、結果の集計などを行うための仕組みです。
...
- CLI オプション（将来的な `-e` 等）や設定ファイルにより、追加のコールバック（JSON 出力用等）を動的に追加可能にします。
...
### 5.2 構造化データ (JSON/YAML) 出力
- マシン読み取り可能な形式での結果出力をサポートするため、`JsonCallback` 等の別実装を提供します。
