...
## 8. 並列実行における出力の最適化 (Optimization for Parallel Execution)

`free` 戦略などの並列実行環境において、複数のホストが同じタスクを開始すると、標準のコールバックでは「TASK [name]」のヘッダーが重複して出力され、ログの可読性が低下します。また、各ホストの実行結果が入り乱れて出力される可能性があります。

`DefaultCallback` では、以下の最適化を行っています。

### 8.1 タスクヘッダーの重複排除
- `ConcurrentHashMap.newKeySet()` を使用して、既に表示されたタスク名をスレッドセーフに管理します。
- `v2_playbook_on_task_start` が呼び出された際、そのタスクが未表示の場合のみヘッダーを出力します。

### 8.2 出力の同期化
- `System.out.println` を含む出力処理を `synchronized` ブロックまたはスレッドセーフな方法で制御し、1つのホストの実行結果（複数行にわたるループ結果など）が他のホストの出力によって分断されないようにします。
