...
## 7. 並列実行における出力の最適化 (Future Design) {#7-並列実行における出力の最適化-future-design}

`free` 戦略などの並列実行環境において、複数のホストが同時に同じタスクを実行する際、標準出力が混み合ったり、同じタスクヘッダーが何度も出力されたりする問題を解決するための設計方針です。

> [!NOTE]
> 現在の `DefaultCallback` は同期処理や重複排除が未実装であり、並列実行時には出力が混在したりヘッダーが重複したりする可能性があります。

### 7.1 タスクヘッダーの重複排除 (Deduplication)
- **現状**: 各ホストがタスクを開始するたびに `v2_playbook_on_task_start` が呼ばれ、ヘッダー（`TASK [name] ***`）が出力されます。
- **課題**: 同一タスクに対するヘッダー出力を 1 回のみに制限する必要があります。
- **実装案**:
    - `Callback` 実装内（特に `DefaultCallback`）で、現在実行中のタスク ID またはオブジェクトをキーとしたスレッドセーフなセット（`ConcurrentHashMap.newKeySet()` 等）を保持します。
    - ヘッダー出力前にセットを確認し、未出力の場合のみ出力してセットに追加します。
    - プレイの開始時等、適切なタイミングでセットをクリアします。

### 7.2 スレッドセーフなコンソール出力
- **方針**: 複数のスレッドからの `System.out.println` 呼び出しによって、一行の出力が途中で混ざり合わない（インターリーブしない）ことを保証します。
- **実装案**:
    - `DefaultCallback` の各メソッド、または出力処理自体を `synchronized` ブロックで保護するか、スレッドセーフなライブラリを介して出力します。
