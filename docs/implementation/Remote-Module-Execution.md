...
### 仕組み
1. **モジュールのパッケージングと依存関係の解消**:
    - **ラッパースクリプトの生成**: 実行対象のモジュール（Python スクリプト）と引数、および実行環境をセットアップするためのラッパースクリプトを生成します。
    - **依存ファイルの集約 (Ansible Core)**: モジュールの実行に不可欠な `ansible.module_utils` および関連するコアファイル (`ansible/__init__.py`, `ansible/release.py`) を ZIP 形式にまとめます。
    - Ansible 本家の Ansiballz 形式を参考にしつつ、モジュール本体とは別に共通の依存関係 ZIP を転送することで、実行効率を高めます。
2. **一時ディレクトリの作成**:
