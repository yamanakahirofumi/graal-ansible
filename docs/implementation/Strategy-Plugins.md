...
### 5.1 シリアル実行 (serial) のサポート
- **概要**: `linear` 戦略において、プレイ内のホストをバッチに分割して実行する機能。
- **実装方針**:
    - `LinearStrategy.run` 内で、ターゲットホストリストを `serial` パラメータ（整数、パーセンテージ、またはリスト）に基づきバッチに分割します。
    - 各バッチの全タスクが完了した後に、そのバッチ内のホストに対してハンドラーをフラッシュ（`flush_handlers`）します。
    - マジック変数 `ansible_play_batch` が、現在実行中のバッチに含まれるホストのみを指すように更新します。
