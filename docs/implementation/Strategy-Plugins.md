...
- **バッチ実行**: `serial` キーワードが指定されている場合、`calculateBatches` メソッドによりターゲットホストをバッチに分割して実行します。各バッチごとに全タスクの完了を待ち、ハンドラーをフラッシュします。
    - **型判定とパーセンテージ計算**: 整数、パーセンテージ文字列（例: `"50%"`）、数値文字列（例: `"2"`）、およびそれらの要素を含むリスト形式に対応します。パーセンテージはターゲットホスト総数から `Math.floor`（最小 1）でサイズを算定します。
    - **リスト指定時の挙動**: リスト形式で指定された場合、各バッチごとに順次サイズが適用されます。バッチ数がリストの要素数を超える場合、リストの**最後の値**が残りのすべてのバッチサイズとして継続的に使用されます。
    - **安全対策フォールバック**: パーセンテージ計算や直接指定により算出されたバッチサイズが `0` 以下の場合、無限ループを回避するため残りのターゲットホスト全員を一つのバッチとして一括処理します。
