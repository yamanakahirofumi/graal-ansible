...
## 6. インベントリのディレクトリサポート (Inventory Directory Support)

単一のファイルだけでなく、ディレクトリを指定してその配下にある複数のインベントリソースを読み込む機能をサポートします。

### 6.1 ディレクトリ走査の挙動
`InventoryManager` がソースとしてディレクトリを受け取った場合、以下のルールに従って再帰的に走査を行います。

1.  **再帰的探索**: 指定されたディレクトリ配下のすべてのサブディレクトリを再帰的に探索します。
2.  **アルファベット順**: 各ディレクトリ内で発見されたファイルは、ファイル名のアルファベット順に処理されます。これにより、複数のファイルで変数が定義されている場合の決定論的な上書き順序を保証します。
3.  **プロバイダーの自動判別**: 発見された各ファイルに対し、登録されている `InventoryProvider` (File, Script 等) を順次適用し、最初に `supports(file)` が `true` を返したプロバイダーで読み込みを行います。

### 6.2 除外対象のファイルとディレクトリ
Ansible 互換の挙動を維持するため、走査中に発見された以下のパターンに一致するファイルおよびディレクトリは無視（スキップ）されます。

- **隠しファイル**: `.` で始まるファイル。
- **バックアップ・一時ファイル**: 以下の拡張子で終わるファイル。
    - `~`, `.bak`, `.old`, `.orig`, `.retry`, `.rpmnew`, `.rpmsave`, `.tmp`
- **特定の予約済みディレクトリ**:
    - `vars`, `group_vars`, `host_vars`
    - これらのディレクトリは変数定義用として別途参照されるため、インベントリソースとしては直接読み込みません。
