...
単一のファイルやスクリプトだけでなく、ディレクトリをインベントリソースとして指定することをサポートします。

### 6.1 探索と読み込みの規則
`InventoryManager` は、ソースとしてディレクトリが渡された場合、以下の規則に従って再帰的に探索を行います。

1.  **再帰的探索**: ディレクトリ内のすべてのファイルを再帰的に探索します。
2.  **アルファベット順**: 発見されたファイルは、パスのアルファベット順にソートして処理されます。
3.  **プロバイダーによる読み込み**: 各ファイルに対し、登録されている `InventoryProvider`（静的ファイル、スクリプト等）を順次適用し、最初に対応したプロバイダーで読み込みます。

### 6.2 除外規則
Ansible の標準仕様に基づき、以下のファイルおよびディレクトリは探索から除外されます。

- **隠しファイル**: `.` で始まるファイル。
- **バックアップ・一時ファイル**: 以下の拡張子で終わるもの。
    - `~`, `.bak`, `.old`, `.orig`, `.retry`, `.rpmnew`, `.rpmsave`, `.tmp`
- **特定のディレクトリ名**:
    - `vars`, `group_vars`, `host_vars`（これらは変数定義用の特別なディレクトリとして別途処理されるため、インベントリソースとしては無視します）。

### 6.3 実装詳細
`InventoryManager.loadInventory` において、`File.isDirectory()` を使用してディレクトリ判定を行い、`Files.walk` または再帰的なメソッド呼び出しによりファイルをリストアップします。リストアップされたファイルは `supports(source)` チェックを経て、既存の `load` ロジックへと渡されます。
