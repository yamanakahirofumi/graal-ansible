...
複数のインベントリソースを一つのディレクトリにまとめて管理することをサポートします。

### 6.1 ディレクトリの再帰的探索
`InventoryManager` にディレクトリパスが渡された場合、その配下を再帰的に探索し、発見されたすべてのファイルをインベントリソースとして処理します。

- **探索順序**: ファイルはアルファベット順に処理されます。
- **除外ルール**: 以下のファイルおよびディレクトリは、インベントリソースとしての探索対象から除外されます。
    - **隠しファイル**: `.` で始まるファイル名。
    - **一時・バックアップファイル**: 以下の拡張子を持つもの。
        - `~`, `.bak`, `.old`, `.orig`, `.retry`, `.rpmnew`, `.rpmsave`, `.tmp`
    - **特定の名前のディレクトリ**:
        - `vars`, `group_vars`, `host_vars`（これらは変数定義用の特殊なディレクトリであるため）。

### 6.2 プロバイダーへの委譲
探索の結果見つかった個々のファイル（静的ファイル、または実行可能なスクリプト）は、登録されている `InventoryProvider`（`FileInventoryProvider`, `ScriptInventoryProvider`）の `supports` メソッドによって適切に振り分けられ、読み込まれます。
