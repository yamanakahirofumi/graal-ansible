...
## 6. インベントリ・ディレクトリ・サポート (Inventory Directory Support)

複数のインベントリソースを一つのディレクトリにまとめて管理し、ディレクトリ単位で指定して読み込む機能をサポートします。

### 6.1 ディレクトリ走査のルール
`InventoryManager` は、指定されたパスがディレクトリである場合、以下のルールに従って再帰的に走査し、発見されたすべての有効なファイルをインベントリソースとして処理します。

- **再帰的走査**: サブディレクトリ内も再帰的に探索します。
- **処理順序**: 発見されたファイルは、ファイル名のアルファベット順に処理されます。これにより、変数の上書き順序を予測可能にします。

### 6.2 除外ルール
Ansible の仕様に基づき、以下のファイルおよびディレクトリは走査の対象から自動的に除外されます。

- **隠しファイル**: `.` で始まるファイル。
- **バックアップ・一時ファイル**: 以下の拡張子を持つファイル。
    - `~`, `.bak`, `.old`, `.orig`, `.retry`, `.rpmnew`, `.rpmsave`, `.tmp`
- **特定のディレクトリ**: 以下の名前のディレクトリ（これらは変数定義用の特別なディレクトリとして扱われるため、インベントリソースとしては無視されます）。
    - `vars`, `group_vars`, `host_vars`

### 6.3 統合メカニズム
各ファイルは、その形式（静的ファイルまたは実行可能スクリプト）に応じて適切な `InventoryProvider` によって読み込まれ、一つの `Inventory` オブジェクトにマージされます。マージの過程で、後から読み込まれたソースの定義が優先されます。
