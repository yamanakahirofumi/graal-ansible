...
## 6. インベントリディレクトリのサポート (Inventory Directory Support)

複数のインベントリソースを一括で読み込むため、ディレクトリの指定をサポートします。

### 6.1 再帰的読み込み
- 指定されたパスがディレクトリである場合、その配下のファイルを再帰的に探索します。
- 探索された各ファイルに対し、登録されている `InventoryProvider` (File, Script等) が順次適用されます。

### 6.2 ファイルフィルタリング
Ansible の標準的な挙動に合わせ、以下の条件に合致するファイルは読み込み対象から除外（無視）されます。

- **隠しファイル**: `.` で始まるファイル名。
- **一時/バックアップファイル**:
    - `~` で終わる。
    - `.bak`, `.old`, `.orig`, `.retry`, `.rpmnew`, `.rpmsave`, `.tmp` で終わる。
- **特定のディレクトリ**: `vars`, `group_vars`, `host_vars` という名称のディレクトリ（これらは変数の読み込みとして別途処理されるため）。
