...
複数のインベントリソースを一つのディレクトリにまとめて指定する機能をサポートします。

### 6.1 ディレクトリの再帰的探索
- 指定されたパスがディレクトリの場合、その配下を再帰的に探索し、発見されたすべてのインベントリソース（ファイルまたは実行可能スクリプト）を読み込みます。
- ファイルの読み込み順序は、ファイル名のアルファベット順とします。

### 6.2 除外ルール
ディレクトリ探索時、以下のファイルおよびディレクトリは無視されます。
- **隠しファイル**: `.` で始まるファイル。
- **バックアップ・一時ファイル**: `~`, `.bak`, `.old`, `.orig`, `.retry`, `.rpmnew`, `.rpmsave`, `.tmp` で終わるファイル。
- **特定のディレクトリ**: `vars`, `group_vars`, `host_vars` という名前のディレクトリ（これらは変数定義用として別途処理されるため）。

### 6.3 実装クラス
- `InventoryManager` クラスがこのディレクトリ探索とプロバイダーへの割り振りロジックを担当します。
