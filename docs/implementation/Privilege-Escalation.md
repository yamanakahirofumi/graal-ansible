...
### 4.3 パスワードのハンドリング (計画中)
- `-K` / `--ask-become-pass` オプションについては、将来的な実装課題として計画されています。
- 実装後は、ユーザーにパスワードをプロンプトで問い合せ、取得したパスワードをメモリ内で安全に保持し、実行時に権限昇格コマンドの標準入力（または `sudo -S` 等の引数）を介して渡す仕組みとなります。
- 現時点では、パスワードを必要としない（パスワードなし sudo が設定されている）環境での利用を前提としています。
