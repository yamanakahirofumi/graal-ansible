...
### 3.1 CLI 変数の注入 (実装済)
`PlaybookCli` において、以下の権限昇格用オプションが実装されています。受け取った引数は「CLI 変数（優先度 Level 1）」として `VariableManager` に登録されます。
...
### 4.3 パスワードのハンドリング (実装済)
- `-K` / `--ask-become-pass` オプションにより、ユーザーにパスワードをプロンプトで問い合せ、取得したパスワードを `ansible_become_password` 変数として保持する仕組みが実装されています。
- 現在、`PlaybookCli` において `System.console()` を介した対話的なパスワード入力がサポートされています。
- 取得されたパスワードは、実行時に権限昇格コマンド（sudo等）で利用可能な状態となります。
