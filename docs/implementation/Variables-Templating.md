...
Ansible 特有のフィルターは、Javaによる独自実装、または基盤となるテンプレートエンジン **Jinjava** の組み込み機能によってサポートされています。現在、計 **25種類** のフィルターが動作確認または実装されています。

### 4.1 実装済みフィルター一覧

以下の 21 種類のフィルターが、Java の `Filter` インターフェースを実装する形で独自に追加されています。
