...
Ansible 特有のフィルターは、Javaによる独自実装、または基盤となるテンプレートエンジン **Jinjava** の組み込み機能によってサポートされています。現在、計 **28種類** のフィルターが動作確認または実装されています。

### 4.1 実装済みフィルター一覧

以下の 24 種類のフィルターが、Java の `Filter` インターフェースを実装する形で独自に追加されています。

- `b64decode`: Base64 デコード。
- `b64encode`: Base64 エンコード。
- `basename`: パスのベース名を取得。
- `bool`: 値を真偽値に変換。
- `comment`: テキストブロックを指定したコメントスタイル（plain/shell, erlang, c, xml）でフォーマット。
- `combine`: 辞書（Map）をマージ。
- `default`: 未定義値に対するデフォルト値を設定.
- `dict2items`: 辞書をリスト形式に変換。
- `dirname`: パスのディレクトリ名を取得。
- `flatten`: ネストされたリストを平坦化。`levels` パラメータによる階層指定をサポート。
- `ipaddr`: IP アドレスの検証・操作（IPv4およびIPv6対応）。
- `items2dict`: リスト形式 of 辞書を一つの辞書に変換。
...
- `to_uuid`: 文字列から標準的なDNSネームスペースを用いた決定論的な UUID v5 を生成。
- `to_yaml`: オブジェクトを YAML 文字列に変換。
- `unique`: リストから重複する要素を排除。
- `urlencode`: URLエンコード処理。
- `urlsplit`: URL/URI を解析してその構成要素（scheme, netloc, path, query, fragment, username, password, hostname, port）を取得。
