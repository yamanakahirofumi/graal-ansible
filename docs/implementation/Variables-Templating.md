...
- **実装済みの主要プラグイン (7種類)** (file, env, template, pipe, dict, vars, first_found):
  ...
  - **使用例**:
    ...
    {{ lookup('file', 'relative/path/to/file.txt') }}
    ```

  #### 2. `env`
  管理ノードの環境変数の値を取得します。環境変数が存在しない場合は空文字列を返します。
  - **使用例**:
    ```jinja2
    {{ lookup('env', 'HOME') }}
    {{ lookup('env', 'PATH') }}
  ...
  #### 3. `template`
  指定された Jinja2 テンプレートファイルを読み込み、現在の変数コンテキストでレンダリングした文字列を返します。
  - **使用例**:
    ```jinja2
    {{ lookup('template', 'templates/config.j2') }}
    ```

  #### 4. `pipe`
  管理ノード上でシェルコマンドを実行し、その標準出力を取得します。
  - **使用例**:
    ```jinja2
  ...
  #### 5. `dict`
  渡された辞書（Map）オブジェクトを、`key` と `value` を要素に持つ Dict 構造体のリスト形式に変換して返します。
  - **使用例**:
    ```jinja2
    {{ lookup('dict', my_map_variable) }}
    ```

  #### 6. `vars`
  指定された変数名の値を動的に取得します。変数が存在しない場合は例外をスローします。
  - **使用例**:
    ```jinja2
