...
- `difference`: 2つのリスト間の差集合を返します。詳細および使用例は [4.3 主要フィルターの詳細仕様](#43-主要フィルターの詳細仕様) を参照。
- `intersect`: 2つのリスト間の積集合を返します。詳細および使用例は [4.3 主要フィルターの詳細仕様](#43-主要フィルターの詳細仕様) を参照。
- `union`: 2つのリスト間の和集合を返します。詳細および使用例は [4.3 主要フィルターの詳細仕様](#43-主要フィルターの詳細仕様) を参照。
- `symmetric_difference`: 2つのリスト間の対称差（排他的な要素）を返します。詳細および使用例は [4.3 主要フィルターの詳細仕様](#43-主要フィルターの詳細仕様) を参照。
...
#### 22. difference / intersect / union / symmetric_difference (集合演算フィルター)
2つのリスト（Collection）間の集合演算（差集合、積集合、和集合、対称差集合）を行い、結果をユニーク要素のリストとして返します。

- **共通パラメータ**:
  - `other` (`Object`, 位置引数 1, 必須): 比較・演算対象となる第 2 のリストまたは Collection オブジェクト。
- **動作仕様および型変換ルール**:
  - **型安全なフォールバック**: 入力オブジェクト（`var`）または演算対象（`other`）が `null` の場合、あるいは Collection / Iterable でない場合は、安全に空のリスト `[]` または要素 1 つのリストにラップして集合演算を処理します。
  - **重複自動排除**: 演算結果は重複要素が取り除かれ（Set セマンティクス）、元の順序を可能な限り維持（`LinkedHashSet` 相当）したリストとして返却されます。
- **各演算の定義と使用例**:
  - **`difference` (差集合)**: 第 1 のリストに存在し、第 2 のリストに含まれない要素を抽出します。
    ```jinja2
    {{ [1, 2, 3, 4] | difference([2, 4, 5]) }}
    {# 返り値: [1, 3] #}
    ```
  - **`intersect` (積集合)**: 双方のリストに共通して存在する要素を抽出します。
    ```jinja2
    {{ [1, 2, 3, 4] | intersect([2, 4, 5]) }}
    {# 返り値: [2, 4] #}
    ```
  - **`union` (和集合)**: 双方のリストの全要素を統合し、重複を除いたリストを生成します。
    ```jinja2
    {{ [1, 2, 3] | union([3, 4, 5]) }}
    {# 返り値: [1, 2, 3, 4, 5] #}
    ```
  - **`symmetric_difference` (対称差)**: 双方のリストのうち、片方にのみ存在する要素（両方に共通しない要素）を抽出します。
    ```jinja2
...
- **プラグインの解決**: 指定されたプラグイン名に基づき、`Lookup` インターフェースを実装した Java 側クラスのレジストリから対応するインスタンスを動的に呼び出します。
- **実装済みの主要プラグイン (7種類)**: `file`, `env`, `template`, `pipe`, `dict`, `vars`, `first_found` の詳細仕様およびパラメータ定義は以下の通りです。
...
| パラメータ | 型 | デフォルト値 | 必須/任意 | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| `terms` | `String` / `List<String>` | なし | 必須 | 読み込み対象のファイルパス（単一文字列、複数位置引数、またはパスのリスト）。 |
| `errors` | `String` | `"strict"` | 任意 | ファイル未検出・読み込み失敗時のエラー処理モード（`strict`, `warn`, `ignore`）。 |

- **パス解決と動作仕様**:
  - 絶対パス（例: `/etc/hosts`）はそのまま読み込みます。
  - 相対パス（例: `files/id_rsa.pub`）は、現在の Jinjava コンテキスト内の `playbook_dir` と結合して絶対パスに自動解決します。
  - `Files.readString(path)` を用いて UTF-8 エンコードのテキストとして全内容を読み込みます。
- **エラーハンドリング**:
  - `strict`: 指定されたファイルが存在しない、または読み込み不可の場合に `RuntimeException` をスローします。
  - `warn` / `ignore`: エラーをスキップし、該当項目の出力をスキップします。
- **使用例**:
  - **単一ファイルの読み込み**:
    ```jinja2
    {{ lookup('file', 'files/id_rsa.pub') }}
    ```
  - **エラー無視モードでの複数ファイル読み込み**:
    ```jinja2
    {{ query('file', 'file1.txt', 'file2.txt', errors='ignore') }}
    ```

#### 2. `env`
管理ノード（制御ノード）の OS システム環境変数の値を取得します。

- **パラメータ仕様**:

| パラメータ | 型 | デフォルト値 | 必須/任意 | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| `terms` | `String` / `List<String>` | なし | 必須 | 取得対象の環境変数名（単一文字列、複数位置引数、または変数名のリスト）。 |
| `default` | `Object` | `null` | 任意 | 指定された環境変数が存在しない場合に返却するフォールバック値。 |

- **動作仕様**:
  - `System.getenv(varName)` を参照して環境変数を検索します。
  - 変数が存在しない場合、`default` パラメータが指定されていればその値を返し、未指定の場合は値を結果リストへ追加しません。
- **使用例**:
  - **基本環境変数の取得**:
    ```jinja2
    {{ lookup('env', 'HOME') }}
  ...
  - **デフォルト値付きでの環境変数取得**:
    ```jinja2
    {{ lookup('env', 'DB_HOST', default='localhost') }}
    ```

#### 3. `template`
指定された Jinja2 テンプレートファイル（`playbook_dir` 相対または絶対パス）を読み込み、現在の変数コンテキストを適用してインライン評価・レンダリングした結果を返します。

- **パラメータ仕様**:

| パラメータ | 型 | デフォルト値 | 必須/任意 | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| `terms` | `String` / `List<String>` | なし | 必須 | テンプレートファイルのパス（単一文字列、複数位置引数、またはパスのリスト）。 |
| `convert_data` | `Boolean` | `true` | 任意 | レンダリング結果が YAML または JSON 構造である場合、自動的に Java の `Map` / `List` オブジェクトへ構造変換するかどうか。 |
| `errors` | `String` | `"strict"` | 任意 | ファイル未検出・レンダリング失敗時のエラー処理モード（`strict`, `warn`, `ignore`）。 |

- **動作仕様**:
  - テンプレートファイルをロードし、`VariableResolver.resolveValue()` を介して現在の Jinjava 変数コンテキスト上で評価（変数展開・フィルター・制御文のレンダリング）を行います。
  - `convert_data=true`（デフォルト）の場合、評価結果の文字列に対し `YamlUtil` による YAML/JSON 解析を試み、成功した場合は構造化オブジェクト（Map / List）として結果リストに格納します。
- **使用例**:
  - **テンプレートファイルのレンダリング**:
    ```jinja2
    {{ lookup('template', 'templates/app.conf.j2') }}
    ```
  - **データ構造への自動変換を無効化したテキストレンダリング**:
    ```jinja2
    {{ lookup('template', 'templates/data.yml.j2', convert_data=false) }}
    ```

#### 4. `pipe`
管理ノード（制御ノード）上で外部シェルコマンドを実行し、その標準出力（`stdout`）を取得します。

- **パラメータ仕様**:

| パラメータ | 型 | デフォルト値 | 必須/任意 | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| `terms` | `String` / `List<String>` | なし | 必須 | 実行するシェルコマンド文字列（単一文字列、複数位置引数、またはコマンドのリスト）。 |
| `errors` | `String` | `"strict"` | 任意 | コマンド実行失敗時のエラー処理モード（`strict`, `warn`, `ignore`）。 |

- **動作仕様**:
  - `OSHandlerFactory.getHandler().getShellExecutable()` により、管理ノードの OS に適した標準シェル（Linux/macOS: `/bin/sh -c`、Windows: `cmd.exe /c`）を用いてコマンドを実行します。
  - 標準出力の末尾の改行コード（`\n`, `\r\n`）および余白文字列は `trim()` により自動的にトリミングされます。
  - 終了コードが非ゼロの場合、`errors='strict'` モードでは標準エラー出力（`stderr`）を含む `RuntimeException` をスローします。
- **使用例**:
  - **日付および Git リビジョンの取得**:
    ```jinja2
...
#### 5. `dict`
渡された辞書（`Map`）オブジェクトのエントリを、`key` と `value` を明示的なキーとして持つ辞書構造体（`{"key": ..., "value": ...}`）のリスト形式に変換して返します。

- **パラメータ仕様**:

| パラメータ | 型 | デフォルト値 | 必須/任意 | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| `terms` | `Map<?, ?>` | なし | 必須 | 変換対象の辞書（`Map`）オブジェクト。 |

- **動作仕様**:
  - 各位置引数の Map オブジェクトをループ走査し、`entrySet()` のキーと値をそれぞれ `"key"` と `"value"` フィールドへ格納した Map のリストを生成します。
  - 引数が Map 型でない場合、`RuntimeException` をスローします。
- **使用例**:
  - **辞書オブジェクトのリスト構造化**:
    ```jinja2
...
#### 6. `vars`
指定された変数名（文字列）に対応する変数の値を、現在の Jinja2 変数コンテキストから動的に検索・取得します。

- **パラメータ仕様**:

| パラメータ | 型 | デフォルト値 | 必須/任意 | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| `terms` | `String` / `List<String>` | なし | 必須 | 参照したい変数名（単一文字列、複数位置引数、または変数名のリスト）。 |
| `default` | `Object` | なし (`null`) | 任意 | 変数がコンテキスト内に存在しない場合に返却するフォールバック値。 |

- **動作仕様**:
  - `JinjavaInterpreter.getContext()` から指定された変数名を検索します。
  - 変数が未定義かつ `default` キーワード引数が指定されている場合、そのデフォルト値を返します。
  - 変数が未定義かつ `default` も未指定の場合、`RuntimeException`（未定義変数エラー）をスローします。
- **使用例**:
  - **動的な変数名の参照**:
    ```jinja2
  ...
  - **デフォルト値を指定した安全な変数参照**:
    ```jinja2
    {{ lookup('vars', 'missing_var', default='fallback_value') }}
    ```
...
| パラメータ | 型 | デフォルト値 | 必須/任意 | 説明 |
| :--- | :--- | :--- | :--- | :--- |
| `terms` / `kwargs` | `List` / `Map` | なし | 任意 | 候補ファイル名 (`files`)、検索ディレクトリ (`paths`)、および非検出時スキップフラグ (`skip`)。 |
| `files` | `List<String>` / `String` | なし | 任意 | 候補ファイル名のリストまたはカンマ区切り文字列。 |
| `paths` | `List<String>` / `String` | なし | 任意 | 検索対象ディレクトリのリストまたはカンマ区切り文字列（`playbook_dir` 相対または絶対パス）。 |
| `skip` | `Boolean` | `false` | 任意 | `true` の場合、候補ファイルが一つも見つからなくても例外をスローせず空のリストを返します。 |

- **動作仕様および候補ファイル検索ルール**:
  - `terms` 内のインライン Map または `kwargs` から `files`, `paths`, `skip` の各パラメータを解析・抽出します（カンマ区切り文字列はリストへ自動分解）。
  - **探索順序**:
    1. 候補ファイル名が絶対パスの場合、直接存在確認 (`Files.isRegularFile`) を行います。
    2. `paths` が空の場合、`playbook_dir/file` の相対パスを検証します。
    3. `paths` が指定されている場合、各ディレクトリ（`playbook_dir` 相対または絶対）と候補ファイル名を結合して順次検証します。
  - 最初に実在が確認できたファイルの絶対パスを文字列として返します。見つからず `skip=false` の場合は `RuntimeException` をスローします。
- **使用例**:
  - **位置引数のリストによる候補指定**:
    ```jinja2
    {{ lookup('first_found', ['custom.conf', 'default.conf']) }}
    ```
  - **`files`, `paths`, `skip` オプションを指定したマップ形式での参照**:
    ```jinja2
    {{ lookup('first_found', {'files': ['app.conf', 'fallback.conf'], 'paths': ['/etc/app', 'config/'], 'skip': true}) }}
    ```

### 7.3 lookup と query の違い
- `lookup`: デフォルトでは複数要素がある場合にカンマ区切りの文字列（例: `"a,b,c"`）を返します。
- `query` (または `wantlist=True` 指定時の `lookup`): 常にリスト形式（例: `['a', 'b', 'c']`）で結果を返します。

### 7.4 共通パラメータとエラーハンドリング仕様

すべての Lookup プラグイン（`lookup` / `query` 関数呼び出し）において、キーワード引数（`kwargs`）経由で指定可能な共通のオプション制御パラメータおよびエラーハンドリングの仕様を以下に定義します。

#### 1. 共通キーワード引数 (Kwargs)

| パラメータ | 型 | デフォルト値 | 説明 |
| :--- | :--- | :--- | :--- |
| `wantlist` | `Boolean` | `false` (`lookup`) / `true` (`query`) | `true` の場合、取得結果を常にリスト型 (`List<Object>`) で返却します。 |
| `errors` | `String` | `"strict"` | エラー発生時の制御方式。`"strict"` (例外スロー), `"warn"` (警告ログ記録後に空リスト返却), `"ignore"` (エラー無視し空リスト返却) のいずれかを指定します。 |
| `default` | `Object` | なし (`null`) | 変数未存在時、環境変数未定義時、またはファイル未検出時にフォールバック値として返却する値。 |
| `convert_data` | `Boolean` | `true` | 読み込んだテキストやテンプレート結果が YAML/JSON 構造である場合に、Java の Map / List オブジェクトへ動的に自動変換するかどうか。 |

#### 2. エラーハンドリング動作 (`errors`)

ファイル未検出、未定義変数参照、コマンド失敗、権限エラーなどが発生した場合、`kwargs` 内の `errors` の値に応じて以下の通り処理されます：

- **`strict` (デフォルト)**:
  - エラーを検知した時点で直ちに `RuntimeException`（または特定のエラークラス）をスローし、タスクの実行を停止（failed）させます。
- **`warn`**:
  - エラー内容を警告ログに記録した上で、例外をスローせず安全に空のリスト `[]`（または `wantlist=false` の場合は空文字 `""`）を返します。
- **`ignore`**:
  - エラーを警告なく無視し、安全に空のリスト `[]`（または空文字 `""`）を返します。

#### 3. 使用例

- **`wantlist` による強制リスト取得**:
  ```jinja2
  {{ lookup('file', 'file1.txt', 'file2.txt', wantlist=true) }}
  {# 返り値: ['content1', 'content2'] #}
  ```

- **`errors='ignore'` による非存在ファイルの安全な読み込み**:
  ```jinja2
  {{ lookup('file', 'maybe_missing.txt', errors='ignore') }}
  {# ファイルが存在しない場合でも例外にならず空文字列または空リストが返る #}
  ```

- **`default` によるフォールバック付き環境変数参照**:
  ```jinja2
  {{ lookup('env', 'NON_EXISTENT_VAR', default='default_value') }}
  {# 返り値: 'default_value' #}
  ```

### 7.5 独自Lookupプラグインの追加手順
