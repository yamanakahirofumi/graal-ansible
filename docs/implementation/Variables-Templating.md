...
- **ハッシュマージ**: 辞書（Map）型の変数について、Ansible 互換の `hash_behaviour=merge` 相当の再帰的マージをサポートしています。
    - **有効化方法**: 環境変数 `ANSIBLE_HASH_BEHAVIOUR=merge` を設定して実行します。
    - **挙動**: デフォルト（`replace`）では、辞書型の変数が重複した場合、優先度の高いレベルの辞書で完全に置き換えられますが、`merge` を指定すると辞書の内容を再帰的にマージします。
