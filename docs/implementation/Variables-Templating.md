...
- **ansible_play_batch** (◎): 現在のシリアルバッチに含まれるホストのうち、まだ失敗していないホストのリスト。`LinearStrategy` における `serial` 指定時、現在のバッチ内のアクティブなホストのみを指すよう `VariableManager.setBatchContext` を通じて動的に更新されます。
