...
- **ハッシュマージ (`hash_behaviour=merge`)**:
    - **現状**: 現在は `replace` 戦略のみをサポートしており、辞書（Map）型の変数が重複した場合、優先度の高いレベルの辞書で完全に置き換えられます。
    - **今後**: Ansible 互換の `hash_behaviour=merge` 相当の再帰的マージのサポートを計画しています。これには環境変数 `ANSIBLE_HASH_BEHAVIOUR` による制御が含まれます。
...
- **ansible_play_batch** (◎): 現在のバッチ（シリアル）に含まれるホストのリスト。`serial` キーワード使用時に、現在の実行バッチに属するホストを動的に反映します。
