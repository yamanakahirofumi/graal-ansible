...
> [!IMPORTANT]
> **現在のステータス (2026年3月)**:
> 現在、`debug`, `set_fact`, `copy`, `template` を含む主要な Action Plugin は、オリジナルの Python 実装をそのまま使用する「Python-first」アーキテクチャで動作しています。Java 版エミュレータは、本家 `ansible-core` への依存や実行時エラーの解消が困難な場合の代替手段となります。

- **本家ansible-coreへの依存**: 本家モジュールの実行にあたり、本家 `ansible-core` に深く依存した処理があり、エミュレーションが困難な場合。
- **解決不能な実行時エラー**: 本家モジュールの実行にあたり、GraalPy 環境特有の制限などにより、実行時のエラーが解消できない場合。
