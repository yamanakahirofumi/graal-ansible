...
> [!IMPORTANT]
> **現在のステータス (2026年3月)**:
> 現在、`debug`, `set_fact`, `copy`, `template` を含む主要な Action Plugin は、オリジナルの Python 実装をそのまま使用する「Python-first」アーキテクチャで動作しています。Java 版エミュレータは、特定の制約下でのバックアップまたは高速化オプションとしての位置付けになっています。
