...
>
> Java による軽量エミュレータは、本家 `ansible-core` に依存する処理がある場合や、実行時のエラーが解消できない場合に限定して利用します。
...
2.  **Java による軽量エミュレータ (特定用途)**:
    - 本家モジュール実行にあたり、本家 `ansible-core` 依存の処理がある場合、または実行時にエラーが解消できない場合について、Java で実装されたエミュレータを使用します。
