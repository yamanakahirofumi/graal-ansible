...
- [リモートノードでのモジュール実行仕様 (GraalPy 委譲)](implementation/Remote-Module-Execution.md)：制御ノード側実行とリモート委譲の仕組み
- [モジュール転送型実行仕様 (Ansible 互換)](implementation/Module-Transfer-Execution.md)：ターゲットノードへモジュールを転送して実行する仕組み
