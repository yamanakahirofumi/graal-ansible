...
| オプション | 短縮形 | 説明 | 優先度 |
| :--- | :--- | :--- | :--- |
| `--inventory` | `-i` | インベントリファイルのパス（ターゲットノードの定義）を指定 | 高 |
| `--extra-vars` | `-e` | 追加の変数を設定 (key=value または JSON/YAML) | 高 |
| `--limit` | `-l` | 実行対象のターゲットノードを制限 | 中 |
| `--tags` | `-t` | 特定のタグが付いたタスクのみ実行 | 中 |
| `--skip-tags` | - | 特定のタグが付いたタスクをスキップ | 低 |
| `--check` | `-C` | 変更を加えずに実行（ドライラン） | 中 |
| `--diff` | `-D` | ファイルの変更内容を表示 | 低 |
| `--verbose` | `-v` | 詳細ログを表示 (`-vvv` 等の複数指定をサポート) | 高 |
| `--become` | `-b` | 権限昇格を有効にする | 高 |
| `--become-method` | - | 権限昇格に使用するメソッドを指定 (sudo, su等) | 中 |
