...
- **仕様**: `loop` および `loop_control` (`index_var`, `loop_var`, `label`, `pause`) をフルサポートしています。
- **状況**: 実装済み。`with_items` などのレガシーな構文も内部で `loop` 処理として統合されています。詳細は [タスク制御の実装詳細](../implementation/Task-Control.md#2-ループ-loop) を参照してください。
