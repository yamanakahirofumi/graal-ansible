...
### 2.6 並列実行制御 (`forks` / `serial`)
- **状況**: ターゲットホストに対して同時に実行するタスクの数（forks）や、一度に実行するホストのバッチ数（serial）を制御する機能が実装されています。
- **詳細**:
    - `forks`: CLI オプション `-f` / `--forks` または設定ファイルにより指定可能です。現状、`FreeStrategy` においてこの設定値が適用され、ホスト間の並列実行を制御します。`LinearStrategy` は各タスクを全ホストに対して順次実行します。
    - `serial`: Playbook 内で指定され、`LinearStrategy` においてホストをバッチ分割して実行します。
...
### 4.1 非同期タスク (`async`, `poll`)
- **状況**: 実装済み。
- **概要**: タスクをバックグラウンドで実行し、完了を待機または後で確認します。
- **詳細**: `AsyncJobManager` によるジョブの状態管理と、`async_status` モジュールによる状況確認をサポートしています。
