...
### 2.5 独自タグの処理
- **状況**: 実装済み。
- **詳細**: `SnakeYAML` のカスタマイズ（`AnsibleYamlConstructor`）により、`!vault`, `!unsafe`, `!unknown_seq` などの未知または未対応のカスタムタグに遭遇した場合、解析エラーで停止させるのではなく、対応するベースの YAML 型（String, List, Map 等）として透過的にフォールバックして処理を継続する仕組みを実装しています。
...
- **詳細**:
    ...
    - `max_fail_percentage`: 失敗したホストの割合がこの閾値を超えた場合に実行を停止します。`LinearStrategy`, `FreeStrategy` およびロール実行時のすべてでサポートされています。
...
### 2.8 ストラテジ固有のコールバック最適化
- **状況**: 実装済み。
- **詳細**: `free` 戦略などの並列実行環境において、標準出力が混み合ったり、同じタスクヘッダーが何度も出力されたりする問題を解決するため、タスク/ハンドラーヘッダーの重複排除（Deduplication）と、スレッドセーフなコンソール出力を実装しています。詳細は [コールバックプラグインの設計仕様](../implementation/Callback-Plugins.md#7-並列実行における出力の最適化-implemented) を参照してください。
