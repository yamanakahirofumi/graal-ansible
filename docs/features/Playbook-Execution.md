...
- **詳細**:
    ...
    - `serial`: Playbook 内で指定され、`LinearStrategy` においてホストをバッチ分割して実行します。整数、パーセンテージ文字列（例: `"50%"`）、数値文字列（例: `"2"`）、およびこれらの要素を含むリスト形式（例: `[1, "40%"]`）をサポートしています。
        - **リスト指定時の挙動**: リスト形式で指定された場合、各要素が順次バッチサイズとして適用されます。リストの要素数よりも多くのバッチが必要な場合は、リストの**最後の値**が残りのすべてのバッチサイズとして継続的に使用されます。
        - **0以下の安全対策フォールバック**: 計算されたバッチサイズが `0` 以下となった場合（例: `0` や `"0%"` 等の指定）、無限ループを回避するため残りの全ターゲットホストを一括して1つのバッチとして処理します。
