...
### 2.6 並列実行制御 (`forks`)
- **状況**: ターゲットホストに対して同時に実行するタスクの数を制御する機能が実装されています。
- **詳細**: CLI オプション `-f` / `--forks` または設定ファイルにより指定可能です。現状、`FreeStrategy` においてこの設定値が適用され、ホスト間の並列実行を制御します。`LinearStrategy` は各タスクを全ホストに対して順次実行します。
