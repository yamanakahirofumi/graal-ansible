...
### 3.3 バッチ実行 (Serial)
- **概要**: `linear` 戦略において、一度に実行するホストの数を制限（ローリングアップデート等）します。
- **指定方法**: 整数（`serial: 2`）、パーセンテージ（`serial: "20%"`）、または段階的なリスト（`serial: [1, 5, 10]`）をサポートします。
- **詳細**: 実装の詳細については、[タスク制御の実装詳細](../implementation/Task-Control.md#18-シリアル実行-serial) を参照してください。
...
### 4.1 並列実行制御 (`forks`)
- **概要**: ターゲットホストに対して同時に実行するタスクの数を制御します。
- **実装方針**: CLI オプション `-f` / `--forks` または設定ファイルにより指定可能にします。

### 4.3 同時実行制限 (`throttle`)
- **概要**: 特定のタスク、ブロック、またはプレイにおいて、同時に実行できるホスト数を制限します。
- **実装方針**: `Task` / `Play` レコードへ `throttle` キーワードを追加します。

### 4.4 再帰的ハッシュマージ (`hash_behaviour=merge`)
