...
### 2.5 バッチ実行 (`serial`)
- **状況**: `linear` 戦略において、一度に実行するホストの数を制限する（バッチ実行）機能が実装されています。
- **動作**:
    - 整数（例: `serial: 2`）またはパーセンテージ（例: `serial: "50%"`）によるバッチサイズの指定が可能です。
    - 各バッチの終了時に、通知されたハンドラーが自動的にフラッシュされます。
    - `ansible_play_batch` マジック変数は、現在実行中のバッチに含まれるホストリストを正しく反映します。
