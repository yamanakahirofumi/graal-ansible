...
- **状況**: `linear`（デフォルト）および `free` 戦略が実装済みです。
- **方針**: `Strategy` インターフェースを介して、Play ごとに実行戦略を切り替えることが可能です。`free` 戦略では、ホストごとの並列タスク実行が実現されています。詳細は [ストラテジ・プラグインの実装仕様](../implementation/Strategy-Plugins.md) を参照してください。
