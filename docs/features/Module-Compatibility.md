...
本プロジェクトでは `ansible-core` に含まれる `ansible.builtin` コレクションの全モジュール (72種類) をサポート対象としています。これらは GraalPy 上で本物の Python コードとして実行され、統合テストにより主要なモジュールのロードおよび初期化が正常に行われることを確認済みです（Linux 環境では 100% ロード可能。Windows/macOS は今後のフェーズで対応予定）。
