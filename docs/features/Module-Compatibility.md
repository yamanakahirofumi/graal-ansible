...
- **Python-first (Primary)**: 本家 Ansible の Python モジュールやコレクションを GraalPy 上でそのまま実行します。ロジックの Java による再実装は行わず、本物の Python コードを使用することで動作の信頼性を確保します。
- **Dependency Emulation Strategy**: GraalPy 上で動作を阻害する `ansible-core` の重厚な依存関係や、ネイティブ拡張を含むモジュールについて、インターフェースを維持したまま最小限のモック化（Dependency Emulation）を行うことで、本物の Python コードの動作を支えます。
