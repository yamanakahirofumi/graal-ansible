...
| :--- | :---: | :---: | :---: | :--- |
| **ansible.builtin** | △ | ？ | ？ | Linux で全 72 モジュールを一覧化。うち 36 モジュールの統合テスト済。 |
...
| - `debconf` | △ | ？ | - | ロード確認済。 |
| - `dnf` | △ | ？ | - | ロード確認済。現在の開発・検証環境（Debian系）では動作確認が困難。 |
| - `dnf5` | △ | ？ | - | ロード確認済。現在の開発・検証環境（Debian系）では動作確認が困難。 |
...
| - `raw` | △ | ？ | ？ | ロード確認済。 |
| - `reboot` | △ | ？ | ？ | ロード確認済。 |
| - `rpm_key` | △ | ？ | - | ロード確認済。現在の開発・検証環境（Debian系）では動作確認が困難。 |
| - `script` | ◎ | ？ | - | オリジナル Python ソースコードにより検証済。 |
| - `service` | △ | ？ | - | ロード確認済。 |
| - `service_facts` | △ | ？ | - | ロード確認済。 |
| - `set_stats` | △ | ？ | ？ | ロード確認済。 |
| - `subversion` | △ | ？ | - | ロード確認済。 |
| - `systemd` | △ | ？ | - | ロード確認済。 |
...
| - `yum_repository` | △ | ？ | - | ロード確認済。現在の開発・検証環境（Debian系）では動作確認が困難。 |
